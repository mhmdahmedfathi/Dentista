...
def Product_Insertion():
    cloumns = ['store_product','STORE_ID','PRODUCT_ID']
    values =[]
    for key in cloumns:
        values.append(request.json[key])
    connector = SQL(host=server_name, user=server_admin)
    PRODUCT_IDColumn = ['PRODUCT_ID']
    condition = "PRODUCT_NAME = '" + values[2]+"'"
    StoreID= connector.select_query(table='product',columns=PRODUCT_IDColumn,sql_condition=condition)
    values[2]=(",".join(repr(e) for e in StoreID['PRODUCT_ID']))
    if values[2]== "":
        return "0"
...
def Product_ID_validator():
    ...
    return validator.email_validation('PRODUCT_ID')
